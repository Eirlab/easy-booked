...
class Build_Calendar {
            ...
            <tr class="abs-week">
                ...
                if ( $day_of_week > 0 ) {
                    for ( $i = 0; $i < $day_of_week; $i++ ) {
                        $time_slot = $this->bookend_status( $previous_date );
                ...
                while ( $current_day <= $number_of_days ) {
                    ...
                    $time_slot = $this->bookend_status( $current_date );
     ...
     * @return array
     */
    public function bookend_status( $date ) {
        $time_slot = array(
            'class' => '',
            'count' => 0,
        );
    ...
    public function get_appointments() {
        ...
        $items = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}abs_custom_time_slots WHERE ( start_date >= %s OR end_date >= %s OR end_date = '' ) AND ( status = 0 ) AND calendar_id = %s",
                $fm_start_date,
                $fm_start_date,
                $calendar_id
            )
        ...
        <div class="abs-time-solat-list">
            <h2><?php echo esc_html__( 'Formation disponible le', 'appointment-booking' ); ?>
            ...
            foreach ( $items as $row ) {
                ...
                if ( strtotime( $appointment_time  ) >= strtotime( $now_time  ) && strtotime($fm_start_date) == strtotime($row->start_date)) {
                    ?>
                    <div class="abs-bookme-timeslot">
                        <div class="abs-bookme-timeslot-left">
                            ...
                            if ( !isset( $abs_calendar_general['display_available_time_slots'] ) ) {
                                ?>
                                <p class="abs-bookme-spaces"><?php echo sprintf( '%s %s', esc_html( $row->space_available ), esc_html__( 'PLACES DISPONIBLES', 'appointment-booking' ) ); ?></p>
                                <?php
                            }
                            ?>
                        </div>
                        <?php
                        if($row->space_available > 0){
                        ?>
                        <div class="abs-bookme-timeslot-right">
                            <a href="" class="abs-bookme-timeslot-button"
                               ...
                               data-slot-id="<?php echo esc_attr( $row->id ); ?>"><?php esc_html_e( 'Inscription', 'appointment-booking' ); ?>
                            </a>
                        </div>
                        <?php
                        }else{
    ...
    public function booked_form_contain() {
        ...
        <div class="abs-booked-header-modal">
            <h5><?php echo esc_html__( 'Inscription à la formation ', 'appointment-booking' ); ?></h5>
    ...
    public function guest_booking_form( $result, $date, $slot_type ) {
        ...
        <form action="" class="abs-book-form-area" method="GET">
            <div class="abs-book-form">
                <h4><?php esc_html_e( 'Vos coordonnées', 'appointment-booking' ); ?>:</h4>
                <?php
                if ( ! isset( $abs_calendar_general['display_name'] ) ) {
                    ?>
                    <div class="abs-book-form-group">
                        <label><?php esc_html_e( 'Nom Prénom', 'appointment-booking' ); ?></label>
            ...
            <div class="abs-book-form-btn">
                ...
                <button class="ab-book-button abs-book-form-btn-success abs-ladda-button abs-slide-right"
                        ...
                        data-slot-id="<?php echo esc_attr( $result->id ); ?>"><span
                            class="label"><?php esc_html_e( 'Inscription ', 'appointment-booking' ); ?></span>
                    <span class="spinner"></span>
                </button>
                <button class="ab-book-button-two abs-book-form-cancel-btn"><?php esc_html_e( 'Annuler', 'appointment-booking' ); ?></button>
            </div>
        </form>
        <?php
    }
        ...
        <form action="" class="abs-book-form-area" method="POST">
            ...
            <div class="abs-book-form-btn">
                ...
                <button class="ab-book-button abs-book-form-btn-success abs-ladda-button abs-slide-right"
                        ...
                        data-slot-type="<?php echo esc_attr( $slot_type ); ?>"
                        data-slot-id="<?php echo esc_attr( $result->id ); ?>">
                    <span class="label"><?php esc_html_e( 'Inscription ', 'appointment-booking' ); ?></span>
                    <span class="spinner"></span>
                </button>
                <button class="ab-book-button-two abs-book-form-cancel-btn"><?php esc_html_e( 'Annuler', 'appointment-booking' ); ?></button>
            </div>
